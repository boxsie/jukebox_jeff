...
class Voice(object):
    ...
    async def play(self, channel, source, title):
        if source.startswith('http') or source.startswith('https'):
            await self._play_url(channel, source, title)
        else:
            await self._play_file(channel, source, title)
    ...
    async def _play_file(self, channel, file_path, title):
    ...
    async def _play_url(self, channel, url, title):
        await self.join_voice_channel(channel)
